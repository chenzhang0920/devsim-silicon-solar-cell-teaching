...
$$
J = \frac{I}{A_{\mathrm{illum}}},
$$

where $A_{\mathrm{illum}}$ is the illuminated active area in $\mathrm{cm^2}$;
the corresponding configuration field is `cell_area_cm2`.
...
photovoltaic open-circuit voltage ($V_{\mathrm{oc}}$), and neither diagnostic is an illuminated
...
positive bias until $|J_{\mathrm{dark}}|$ is comparable with $|J_{\mathrm{sc}}|$, or stop at the approved compliance or
...
- add `--prune` only when `--data` names a complete measurement bundle. It validates the
  required illuminated/dark J–V and illuminated $V_{\mathrm{oc}}$/short-circuit files, then removes stale
...
$0\le V\le V_{\max}$ (0.72 V by default, set by `--v-max`). Reverse-bias rows remain unchanged in the raw
