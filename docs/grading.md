...
Assessment conclusions must be supported by the dataset assigned to the student's group.
Reproducing the bundled Cell #3 results or its reference interpretation does not earn credit
for group-specific evidence or analysis.
...
| Criterion | Points |
|---|---:|
| Presents a readable J–V plot and correctly reports $J_{\mathrm{sc}}$, $V_{\mathrm{oc}}$, FF, and $P_{\max}$ with units | 6 |
...
| Presents the modeled EQE and interprets its relationship to the single-pass absorption reference and integrated $J_{\mathrm{sc}}$ | 5 |
