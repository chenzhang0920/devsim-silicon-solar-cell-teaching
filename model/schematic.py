...
def _draw_device(ax: plt.Axes, p: SolarCellParams) -> None:
    ...
    ax.text(9.35, 6.0, r"depth $x$", fontsize=11.5, va="center", rotation=90, color=C_DARK)
...
def _draw_band(ax: plt.Axes, d: dict, p: SolarCellParams, bias: float) -> None:
    ...
    ax.plot(x, Ec_eq, color=C_BLUE, lw=1.6, label=r"$E_{\mathrm{c}}$ (equil.)")
    ax.plot(x, Ev_eq, color=C_ORANGE, lw=1.6, label=r"$E_{\mathrm{v}}$ (equil.)")
    ax.plot(x, Ec_i, color=C_BLUE, lw=1.4, ls="--", label=r"$E_{\mathrm{c}}$ (illum.)")
    ax.plot(x, Ev_i, color=C_ORANGE, lw=1.4, ls="--", label=r"$E_{\mathrm{v}}$ (illum.)")
    ax.plot(x, Efn_i, color=C_BLUE, lw=1.2, ls=":", label=r"$E_{\mathrm{F}n}$")
    ax.plot(x, Efp_i, color=C_ORANGE, lw=1.2, ls=":", label=r"$E_{\mathrm{F}p}$")
    ax.axvline(p.emitter_depth * 1e4, color="k", ls=":", lw=0.8)
    ax.set_xlim(0, 2)
    ax.set_ylim(-1.4, 1.4)
    ax.set_xlabel(r"Depth $x$ ($\mu\mathrm{m}$)")
...
def _draw_iv(ax: plt.Axes, V, J, pin: float) -> None:
    ...
    ax.set_xlabel(r"Voltage $V$ ($\mathrm{V}$)")
    ax.set_ylabel(r"Current density $J$ ($\mathrm{mA\,cm^{-2}}$)")
    ax.set_title("③ Simulated J–V curve and\nmaximum-power point", fontsize=15, pad=8)
    ax.set_ylim(-5.0, 32.0)
    ax.grid(alpha=0.3)
    text = (f"$J_{{\\mathrm{{sc}}}}={m['Jsc']*1e3:.2f}\\ \\mathrm{{mA\\,cm^{{-2}}}}$\n"
            f"$V_{{\\mathrm{{oc}}}}={m['Voc']:.3f}\\ \\mathrm{{V}}$\n"
            f"FF = {m['FF']:.3f}\n"
