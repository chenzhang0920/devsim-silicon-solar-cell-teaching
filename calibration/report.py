...
def comparison_figure(
    ...
    ax1.set_ylabel(r"Current density $J$ ($\mathrm{mA\,cm^{-2}}$)")
    ax1.set_title(f"Fit to the {data_label} J–V curve")
    ax1.legend(frameon=False, loc="upper right")

    text = (
        f"$J_{{\\mathrm{{sc}}}}$: data {m_meas['Jsc'] * 1e3:.2f}, "
        f"model {m_sim['Jsc'] * 1e3:.2f} $\\mathrm{{mA\\,cm^{{-2}}}}$\n"
        f"$V_{{\\mathrm{{oc}}}}$: data {m_meas['Voc']:.3f}, "
        f"model {m_sim['Voc']:.3f} $\\mathrm{{V}}$\n"
        f"$\\mathrm{{FF}}$: data {m_meas['FF']:.3f}, model {m_sim['FF']:.3f}\n"
        f"$P_{{\\mathrm{{max}}}}$: data {m_meas['Pmax'] * 1e3:.2f}, "
        f"model {m_sim['Pmax'] * 1e3:.2f} $\\mathrm{{mW\\,cm^{{-2}}}}$"
    )
    ...
    ax2.set_xlabel(r"Voltage $V$ ($\mathrm{V}$)")
    ax2.set_ylabel(r"Model − data ($\mathrm{mA\,cm^{-2}}$)")
    rmse = float(np.sqrt(np.mean(resid ** 2)))
    ax2.set_title(f"Residuals: $\\mathrm{{RMSE}}={rmse:.2f}\\ \\mathrm{{mA\\,cm^{{-2}}}}$")
...
def joint_comparison_figure(
    ...
    ax.scatter([data.light_ishort["V_mean_V"]],
               ...
               color=C_GREEN, marker="D", s=45, label=r"$J_{\mathrm{sc}}$ check", zorder=4)
    ax.axvline(data.light_voc, color=C_YELLOW, ls="--", lw=1.5,
               label=f"$V_{{\\mathrm{{oc}}}}={data.light_voc:.3f}\\ \\mathrm{{V}}$")
    ax.axhline(0.0, color=C_GRAY, lw=0.9)
    ax.set_title(f"Cell #{data.sample}: illuminated J–V", fontsize=18)
    ax.set_xlabel(r"Terminal voltage $V$ ($\mathrm{V}$)", fontsize=16)
    ax.set_ylabel(r"Current density $J$ ($\mathrm{mA\,cm^{-2}}$)", fontsize=16)
    ...
    if data.dark_open_circuit_offset is not None:
        diagnostic_lines.append(
            f"zero-current offset: ${data.dark_open_circuit_offset:.3f}\\ \\mathrm{{V}}$"
        )
    if data.dark_ishort is not None:
        diagnostic_lines.append(
            f"$J$ near $0\\ \\mathrm{{V}}$: "
            f"${data.dark_ishort['J_mean_A_cm2'] * 1e3:.3f}\\ \\mathrm{{mA\\,cm^{{-2}}}}$"
        )
    if diagnostic_lines:
        ax.text(
            0.03, 0.96,
    ...
    if block_diagnostics:
        ...
        labels = {
            "light_iv": "Light J–V",
            "dark_iv": "Dark J–V",
            "light_ishort": r"$J_{\mathrm{sc}}$ anchor",
            "light_voc": r"$V_{\mathrm{oc}}$ anchor",
        }
