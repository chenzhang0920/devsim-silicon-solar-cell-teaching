...
def test_physics_polarity_units_and_jv_language_are_explicit():
    ...
    assert r"\varepsilon_{\mathrm{Si}}" in markdown
    assert r"N_{\mathrm D}^{+}-N_{\mathrm A}^{-}" in markdown
    assert "$N_{\\mathrm D}-N_{\\mathrm A}<0$ (p⁺ emitter)" in structure
...
def test_notebook_uses_portable_setup_and_the_configured_reporting_grid():
    ...
    assert "teaching aids, not statistical acceptance gates" in code
