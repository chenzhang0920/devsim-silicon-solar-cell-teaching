...
def plot(V: np.ndarray, J_mA: np.ndarray,
    ...
    if np.isfinite(eta):
        text = (f"$J_{{\\mathrm{{sc}}}}={jsc:.2f}\\ \\mathrm{{mA\\,cm^{{-2}}}}$   "
                f"$V_{{\\mathrm{{oc}}}}={voc:.3f}\\ \\mathrm{{V}}$   FF={ff:.3f}\n"
                f"$P_{{\\max}}={pmax:.2f}\\ \\mathrm{{mW\\,cm^{{-2}}}}$   "
                f"$\\eta_{{\\mathrm{{model}}}}={eta:.2f}\\%$   "
                f"MPP={vmp:.3f} V, {jmp:.1f} mA/cm²")
    else:
        text = (f"$J_{{\\mathrm{{sc}}}}={jsc:.2f}\\ \\mathrm{{mA\\,cm^{{-2}}}}$   "
                f"$V_{{\\mathrm{{oc}}}}={voc:.3f}\\ \\mathrm{{V}}$   FF={ff:.3f}\n"
                f"$P_{{\\max}}={pmax:.2f}\\ \\mathrm{{mW\\,cm^{{-2}}}}$   "
                f"modeled $\\eta$ omitted ($P_{{\\mathrm{{in}}}}$ unavailable)\n"
                f"MPP={vmp:.3f} V, {jmp:.1f} mA/cm²")
    ...
    ax.text(0.99, 0.98, r"generation-positive $J$; output power density $P=VJ$",
