...
PARAM_LABELS = {
    "electron_lifetime": r"electron lifetime $\tau_n$ ($\mathrm{s}$)",
    "hole_lifetime": r"region-wide hole lifetime $\tau_p$ ($\mathrm{s}$)",
    "front_srv": r"effective front loss velocity ($\mathrm{cm\,s^{-1}}$)",
    "back_srv": r"effective rear loss velocity ($\mathrm{cm\,s^{-1}}$)",
    "emitter_doping": r"p⁺ emitter doping $N_{\mathrm{A}}$ ($\mathrm{cm^{-3}}$)",
    "base_doping": r"n-base doping $N_{\mathrm{D}}$ ($\mathrm{cm^{-3}}$)",
    "thickness": r"thickness ($\mu\mathrm{m}$)",
...
def main() -> None:
    ...
    ax1.set_ylabel(r"$V_{\mathrm{oc}}$ ($\mathrm{V}$)", fontsize=16)
    ...
    ax2.set_ylabel(r"$J_{\mathrm{sc}}$ ($\mathrm{mA\,cm^{-2}}$)", fontsize=16)
