...
def main() -> None:
    ...
    for k, Rs in enumerate(Rs_list):
        ...
        label = (f"baseline: $R_{{\\mathrm{{s}}}}=0$ (FF = {ff:.3f})" if Rs == 0
                 else f"$R_{{\\mathrm{{s}}}}={Rs:.0f}\\ \\Omega\\!\\cdot\\!\\mathrm{{cm^2}}$  (FF = {ff:.3f})")
        ax1.plot(
            Vs, J, lw=2.2, ls=line_styles[k], label=label,
            color=(C_DARK, C_BLUE, "#6a3d9a")[k],
        )
    ax1.axhline(0, color="gray", lw=0.8)
    ax1.set_xlabel(r"Voltage $V$ ($\mathrm{V}$)")
    ...
    for k, Rsh in enumerate(Rsh_list):
        ...
        label = (f"baseline: $R_{{\\mathrm{{sh}}}}=\\infty$; $V_{{\\mathrm{{oc}}}}={vs:.3f}\\ \\mathrm{{V}}$; FF={metrics['FF']:.3f}" if np.isinf(Rsh)
                 else f"$R_{{\\mathrm{{sh}}}}={Rsh:.0f}\\ \\Omega\\!\\cdot\\!\\mathrm{{cm^2}}$; $V_{{\\mathrm{{oc}}}}={vs:.3f}\\ \\mathrm{{V}}$; FF={metrics['FF']:.3f}")
        ax2.plot(
    ...
    ax2.set_ylabel(r"Current density $J$ ($\mathrm{mA\,cm^{-2}}$)")
    ax2.set_title(r"Shunt leakage reduces $V_{\mathrm{oc}}$ and fill factor")
