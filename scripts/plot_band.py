...
def _style(ax, title, *, show_ylabel=True):
    ...
    ax.set_xlabel(r"Depth $x$ ($\mu\mathrm{m}$) — front-junction zoom", fontsize=16)
    if show_ylabel:
        ax.set_ylabel(r"Energy ($\mathrm{eV}$)", fontsize=16)
...
def main() -> None:
    ...
    ax1.plot(x, Ec_eq, color=C_BLUE, lw=1.8, label=r"$E_{\mathrm{c}}$")
    ax1.plot(x, Ev_eq, color=C_ORANGE, lw=1.8, label=r"$E_{\mathrm{v}}$")
    ax1.plot(x, np.zeros_like(x), color=C_DARK, lw=1.2, ls="--", label=r"$E_{\mathrm{F}}$")
    ...
    ax1.text(1.86, (Ec_eq[i_n] + y_top) / 2,
             f"$qV_{{\\mathrm{{bi}}}}={vbi:.2f}\\ \\mathrm{{eV}}$",
             fontsize=14.5, va="center")
    ax1.legend(fontsize=14, frameon=False, loc="lower right")
    _style(ax1, "Equilibrium: one flat Fermi level")


    ax2.plot(x, Ec_i, color=C_BLUE, lw=1.8, label=r"$E_{\mathrm{c}}$")
    ax2.plot(x, Ev_i, color=C_ORANGE, lw=1.8, label=r"$E_{\mathrm{v}}$")
    ax2.plot(x, Efn_i, color=C_BLUE, lw=1.2, ls="--", label=r"$E_{\mathrm{F}n}$")
    ax2.plot(x, Efp_i, color=C_ORANGE, lw=1.2, ls="--", label=r"$E_{\mathrm{F}p}$")
    ...
    ax2.text(sample_x + 0.06, (Efn_i[i_mid] + Efp_i[i_mid]) / 2,
             f"local $\\Delta E_{{\\mathrm{{F}}}}={delta_ef:.2f}\\ \\mathrm{{eV}}$",
             fontsize=14.5, va="center")
